...
The previous gate measured accuracy among all predictions whose maximum class probability was at least 85%. Because ±2% moves inside 15–60 minutes are rare, high-confidence `NO_EVENT` predictions could dominate that sample and allow the model to pass without demonstrating useful precision for either directional event.

That behavior contradicted the project rule that overall accuracy and `NO_EVENT` dominance cannot qualify the ±2% First-Touch Model.
...
A champion is accepted only when untouched temporal test data contains sufficient support for both `UP_02` and `DOWN_02`, and the model produces enough high-confidence predictions for each direction and in total, with at least 85% empirical precision across those directional predictions.
...
- directional results for `UP_02` and `DOWN_02` separately.

Trading readiness remains `WAIT` regardless of research-monitoring status.

## Interpretation

A probability near 100% for `NO_EVENT` may be statistically plausible for a rare ±2% target, but it is not evidence that the platform can identify impending +2% or -2% shocks. The corrected gate prevents that distinction from being hidden.